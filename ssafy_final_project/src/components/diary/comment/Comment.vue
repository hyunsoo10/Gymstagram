      ...
      <v-dialog width="500">
        ...
        <template v-slot:default="{ isActive }">
          <v-card title="답글 작성하기">
            <v-card-text>
              <v-text-field @keyup.enter="createSubComment(comment), isActive.value = false" v-model="subCommentContent"
                      ...
                      <v-card title="댓글 수정하기">
                        <v-card-text>
                          <v-text-field :placeholder="subComment.content"
                            @keyup.enter="updateComment(subComment), isActive.value = false" v-model="updateContent"
                            type="text"></v-text-field>
                        </v-card-text>

                        <v-card-actions>
                          <v-spacer></v-spacer>
                          <v-btn text="수정완료" @click="updateComment(subComment), isActive.value = false"></v-btn>
                          <v-btn text="취소" @click="isActive.value = false"></v-btn>
                        </v-card-actions>
                      </v-card>
                    </template>
                  </v-dialog>
                  <v-icon icon="mdi-close-circle" v-bind="props" v-if="userStore.loginUser.userId == subComment.userId"
      ...
      <template v-if="userStore.loginUser != null && (userStore.loginUser.userId == comment.userId)">
        <v-dialog width="500">
          ...
          <template v-slot:default="{ isActive }">
            <v-card title="댓글 수정하기">
              <v-card-text>
                <v-text-field :placeholder="comment.content" @keyup.enter="updateComment(comment), isActive.value = false"
